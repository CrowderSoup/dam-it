...
The immediate implementation paired with this audit replaces Reed's
whole-body bob and mirrored two-direction drawing with four readable cardinal
poses and a layered procedural walk cycle. Audio structure and final UI polish
remain deliberately specified rather than wired into story code that issues
#17, #20, and #21 still need to establish.
...
| P1 | Final dialogue, journal, and chapter-ending copy do not exist yet, so a conclusive scale/contrast pass would be testing placeholder density. | Defer final adjustment until #17 and #21 have representative longest strings. Add bounds/focus assertions with each content PR. | Required before #9 closes. |
| P1 | The game has procedural effects but no ambient or musical state model. Wiring cues directly to current demo events would make dialogue/save transitions fragile. | Use the cue-state contract below after #17 publishes narrative milestones. | Required before #9 closes. |
...
## Audio state contract for the next #9 slice
