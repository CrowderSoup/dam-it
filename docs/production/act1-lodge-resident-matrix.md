...
| Stage 1 → 2; enclosed walls / dry storage | The Lodge visibly gains protected storage | No new resident is caused by this stage | Moss uses the platform less often and returns to pond cover; eligible Eddy continues using the water route independently | A short Lodge-storage follow-up is available from the relevant resident chosen by #17; no mandatory interruption | Moss now expects Reed to remain; Eddy's trust still depends on the flow solution, not this wall | **Reinforced Pouch** becomes purchasable as one optional upgrade; remove the prototype's three escalating tiers when this row is implemented | Still disabled | `lodge_stage == 2`, single pouch-upgrade state, and one-time dialogue flag | Stage two unlocks exactly one optional pouch purchase; it never gates story; load cannot duplicate the purchase or regress the resident line |
| Stage 2 → 3; finished roof / warm center | The finished Lodge becomes a community gathering place, distinct from the dam payoff | No resident teleports in solely because the roof is built | Moss can pause at the Lodge and pond; Eddy remains on the overflow route when his flow gate is satisfied | Final Moss Lodge line becomes available; completing it contributes to chapter-ending eligibility | Moss treats Reed's home as part of Willowbend and Reed as a friend who is staying | No new traversal yet; stage three unlocks **celebration eligibility**, not chapter completion itself | Authored introduction may become eligible only after all chapter prerequisites below are true | `lodge_stage == 3`, Moss Lodge acknowledgement flag, and #17 prerequisite flags/objectives | Roof completion cannot directly complete the chapter; stage-three save restores calm play and eligibility; celebration cannot begin if Eddy's flow beat or required Lodge acknowledgement is incomplete |
| Chapter-ending sequence; owned jointly with #21 | Lodge gathering visually pays off all three stages | Marnie arrives from upstream in response to strange Aspen Meadow water; Bramble appears only in the authored encounter | Moss gathers at the Lodge, Eddy uses restored flow, and Marnie approaches from the upstream route; Bramble follows a deterministic enter/take/flee path | #21 starts the celebration only after the eligibility contract; #17 supplies Marnie, muddy-water, Moss/Bramble-reaction, and transition text | Moss's quick judgment of Bramble creates future friction; Marnie establishes the upstream need; Bramble's chosen materials and charred/debris context suggest shelter-building without excusing the harm | Upstream route is revealed by the explicit chapter-complete event, not by Lodge stage | One authored encounter runs once. Its fixed, clamped loss is committed once before Bramble flees; absence of resources cannot block the scene | `bramble_intro_started`, `bramble_material_taken`, `bramble_intro_complete`, and #21's separate `act_one_complete` campaign flag; active dialogue remains in the existing story payload | Every prerequisite combination is covered; the sequence cannot trigger twice; saving at each discrete flag boundary converges to a valid state; fixed loss cannot go negative or consume materials still required by an active objective |
| Calm post-chapter / repeat play | Rest and the optional single pouch upgrade remain available | Moss, Eddy, and Marnie remain present in their established habitats until Act II exists | Each resident resumes a short routine; Marnie waits near the upstream route rather than occupying the Lodge | Interactions choose concise post-chapter ambient lines and never restart required scenes | Willowbend reads as a community, while Bramble remains an unresolved neighbor rather than a defeated nuisance | Route hook remains visible; no further capability is required in Milestone 1 | Recurring scavenging may be enabled only if a separate playtest decision keeps it; the hard minimum is `bramble_intro_complete == true` | Story flags, `act_one_complete`, Lodge/pouch state; routine waypoint/animation phase is not saved | Loading a completed save is calm, residents are interactive, required dialogue does not replay, and any retained random scavenging is impossible on pre-introduction saves |
