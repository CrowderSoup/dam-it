...
func _ready() -> void:
	...
	var fresh_marnie: Resident = fresh_main.get_node("Residents/Marnie")
	...
	assert(fresh_eddy.dialogue_ids.size() == 2, "Eddy's nested scene instance must retain every authored dialogue id")
	assert(fresh_marnie.dialogue_ids.size() == 2, "Marnie's nested scene instance must retain every authored dialogue id")
